<?php
    ...
    $statement->execute(array('cote'=> $cote1));
	...
	if(!empty($_POST))
	{
		...
		if($isSuccess) 
        {
            $statement = $bdd->prepare("UPDATE document SET cote=:cote ,titre=:titre ,isbn=:isbn ,type=:type ,resume=:resume ,langue=:langue ,auteur=:auteur ,quantite=:quantite ,disponibilite=:disponibilite ,date_edition=:date_edition ,domaine=:domaine WHERE document.cote=:cote1");
            ...
            header("Location: document-voir.php?cote=$cote");
        }
	}	
?>
<!DOCTYPE html>
...
<head>
	...
	<title>Modification d'Un Document -Administrateur</title>
</head>
<?php
		...
		<div class="wrraper2">
			<div class="container2">
				<div class="row">
					...
					<br>
					<?php
					$statement = $bdd->prepare("SELECT cote FROM document WHERE document.cote = :cote");
				    $statement->execute(array('cote'=> $_GET['cote']));
				    $item = $statement->fetch();
				    ?>

					<form method="post" action="document-modifier.php?cote=<?php echo$item['cote'];?> " role="form" enctype="multipart/form-data">
                        ...
                        <select class="form-control" id="type" name="type">
                               ...
                               <option value="rapport de stage">Raport De Stage</option>
                               <option value="these">Thèse</option>
                               <option value="dictionnaire">Dictionnaire</option>
                               <option value="cd memoire">CD Mémoire</option>
						...
						<div class="form-group">
	                        ...
	                        <input type="date" class="form-control" id="date_edition" name="date_edition" placeholder="Date_Edition" value="<?php echo $date_edition;?>">
